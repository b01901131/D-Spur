...
void Control_System::Obstacle_Avoidance(){
     ...
     int information;
     information=Detect_Obstacle();
     
     //Reaction for the obstacle.
     Reaction_for_Obstacle(information);
}

void Control_System::Detect_and_Pass_Seesaw(){
    //Detect Seesaw from IMU
    int Information_Seesaw;
    Information_Seesaw=IMU_Detect();
    
    //React
    //We should use the break.
    //Otherwise, the car will go out of the lane.
    Reaction_for_Seesaw(Information_Seesaw);
}
